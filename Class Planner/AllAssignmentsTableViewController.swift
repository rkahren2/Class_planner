...
class AllAssignmentsTableViewController: UITableViewController, NSFetchedResultsControllerDelegate{

    
    @IBOutlet weak var noAllAssignmentsLabel: UILabel!          //no assignments message
    
    //array for assignments
    var assignments:[Assignment] = []
    
    //create managedObjectContext, fetechResultController, and predicate for retriving from CoreDat
    ...
    ************************************************************************************/
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if assignments.count == 0{
            noAllAssignmentsLabel.hidden = false
            tableView.backgroundView = noAllAssignmentsLabel
            tableView.separatorStyle = UITableViewCellSeparatorStyle.None
            return 0
        //otherwise hide no note message and return the number of notes
        }else{
            noAllAssignmentsLabel.hidden = true
    ...
    ************************************************************************************/
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("AllAssignmentsCell", forIndexPath: indexPath) as! AllAssignmentsTableViewCell
        
        // Configure the cell
        let today = NSDate()
        let assignment = assignments[indexPath.row]
        cell.assignmentTitleLabel.text = assignment.title
        ...
        if assignment.dueDate.compare(today) == .OrderedAscending{
            cell.assignmentTitleLabel.textColor = UIColor.redColor()
...
//      NSFetchedResultsChangeType, newIndexPath: NSIndexPath!) {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        if segue.identifier == "viewAssignment" {
            if let indexPath = self.tableView.indexPathForSelectedRow() {
                ...
                // Passing the restaurant object to the detail view
    ...
    ************************************************************************************/
    @IBAction func unwindToAllAssignment(segue:UIStoryboardSegue) {
    }
